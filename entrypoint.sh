...
HTTP_DOMAINS=$(echo "$NGINX_PROXY_PASS"     | python gen.py http)
HTTPS_DOMAINS=$(echo "$NGINX_PROXY_PASS"    | python gen.py https)
CERBOT_DOMAINS=$(echo "$NGINX_PROXY_PASS"   | python gen.py certbot)
NGINX_UPSTREAMS=$(echo "$NGINX_PROXY_PASS"  | python gen.py upstreams)
